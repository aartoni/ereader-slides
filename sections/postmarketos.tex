...
\begin{frame}{Fix: touchscreen}
    ...
    \pause
    Looks like someone has fucked up the DTS for the Kobo Clara HD.
\end{frame}

\begin{frame}[fragile]
    \frametitle{Device Tree Source}
    \begin{verbatim}
&cpu0 {
...
\begin{frame}{lisgd}
    I can receive touch events via \texttt{evtest}, but still no response on screen.

    \pause
    SXMO handles gestures via a daemon called \texttt{lisgd}.

    \pause
    \texttt{lisgd} is having troubles attaching to the right X.Org instance/display.
\end{frame}

\begin{frame}{I can finally use SXMO!}
    \centering
    \includegraphics{assets/clara/sxmo}
\end{frame}

\begin{frame}{I can finally use Firefox!}
    \centering
    \includegraphics[scale=0.25]{assets/clara/firefox}
\end{frame}
